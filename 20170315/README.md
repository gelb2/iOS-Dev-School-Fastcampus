### 20170315
### 네트워크 기초
* 네트워크는 어떤 연결을 통해 컴퓨터의 자원을 공유하는 것

## 클라이언트 서버 모델.   
- 네트워크 아키텍쳐 중 하나.   
- 서버 : 클라이언트의 요청에 따라 데이터를 제공해 주는 컴퓨터 == 백엔드.   
- 클라이언트 : 서버로부터 요청한 데이터를 받는 컴퓨터 == 프론트엔드, iOS 개발자 
...
* 각각의 컴퓨터가 클라이언트, 서버 역할에 맞게 구성되고 네트워크 통신이 이뤄진다면 그게 곧 클라이언트 서버 모델임
...
* Message : HTTP 메소드를 사용함 (GET, POST, PUT, PATCH, DELETE).   
...
1. info.plist 파일 열기 
2. 새로운 Row 추가
3. App Transport Security Setting 항목 추가
4. Allow Arbitrary Loads 추가
5. YES로 변경 : ATS 해제…….해제 옵션은 도메인 등 다양함